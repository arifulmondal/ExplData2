        ...
        # Subset only for Coal Combustion
        coal_combustion<-as.data.frame(SCC[grepl("coal",SCC$EI.Sector,ignore.case=TRUE) &
                                         grepl("combustion",SCC$SCC.Level.One,ignore.case=TRUE) 
                                           ,1])
        colnames(coal_combustion)<-"SCC"
        ...
        total_PM25<-ddply( NEI_CC, .(year), summarize, total_emission=sum(Emissions))
        head(total_PM25)
        ...
        png("plot4.png")
        ggplot(data=total_PM25, aes(x=year, y=total_emission)) + geom_line() + geom_point() + labs(title="PM2.5 Emissions from Sources Related to Coal-Combustion")
        dev.off()
