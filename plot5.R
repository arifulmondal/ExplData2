        ...
        NEI_Baltimore_MVH<-merge(NEI_Baltimore, motor_vh, by="SCC" )
        ...
        total_PM25<-ddply( NEI_Baltimore_MVH, .(year), summarize, total_emission=sum(Emissions))
        head(total_PM25)
        ...
        png("plot5.png")
        ggplot(data=total_PM25, aes(x=year, y=total_emission)) + geom_line() + geom_point() + labs(title=" Baltimore PM2.5 Emissions from Sources Related to Motor Vehicle")
        dev.off()
